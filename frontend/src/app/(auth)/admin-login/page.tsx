...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const redirectUrl = loggedUser.role === 'CUSTOMER' ? '/' : '/admin'
    ...
    <div className="min-h-[calc(100vh-128px)] flex items-center justify-center px-4 py-12 bg-surface">
      <div className="w-full max-w-md glass rounded-3xl p-8 animate-fade-in">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-muted mt-1 font-medium">Store Operations & Back-Office</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-muted mb-1.5 block">Email or Username</label>
            <input
